...
```{r}
geneList6h <- data_results$Ri6h_vs_Rm6h_p.adj
...
GOdata6h <- new("topGOdata",
              ontology = "BP",
              allGenes = geneList6h,
              geneSelectionFun = function(x) (x < 0.1),
...
```{r}
geneList12h <- data_results$Ri12h_vs_Rm12h_p.adj
...
```{r}
geneList24h <- data_results$Ri24h_vs_Rm24h_p.adj
...
for (x in 1:sum(tab6h$raw.p.value < 0.05)) {
  ...
  writeFormula(excel, "Summary", startRow = x+1, 
               x = makeHyperlinkString(sheet = sheetname, row = 1, col = 1
                                       , text = goID))
  conditionalFormatting(excel, sheetname, cols=18, rows=2:nrow(GoDF[[x]]), rule=upreg, style = posStyle)
  conditionalFormatting(excel, sheetname, cols=18, rows=2:nrow(GoDF[[x]]), rule=downreg, style = posStyle)
  conditionalFormatting(excel, sheetname, cols=8, rows=2:nrow(GoDF[[x]]), rule="<0.05", style = posStyle)
  conditionalFormatting(excel, sheetname, cols=11, rows=2:nrow(GoDF[[x]]), rule="<0.05", style = posStyle)
}
...
for (x in 1:sum(tab12h$raw.p.value < 0.05)) {
  ...
  writeFormula(excel, "Summary", startRow = x+1, 
               x = makeHyperlinkString(sheet = sheetname, row = 1, col = 1
                                       , text = goID))
  conditionalFormatting(excel, sheetname, cols=16, rows=2:nrow(GoDF[[x]]), rule=upreg, style = posStyle)
  conditionalFormatting(excel, sheetname, cols=16, rows=2:nrow(GoDF[[x]]), rule=downreg, style = posStyle)
  conditionalFormatting(excel, sheetname, cols=6, rows=2:nrow(GoDF[[x]]), rule="<0.05", style = posStyle)
  conditionalFormatting(excel, sheetname, cols=9, rows=2:nrow(GoDF[[x]]), rule="<0.05", style = posStyle)
}
...
for (x in 1:sum(tab24h$raw.p.value < 0.05)) {
  ...
  writeFormula(excel, "Summary", startRow = x+1, 
               x = makeHyperlinkString(sheet = sheetname, row = 1, col = 1
                                       , text = goID))
  conditionalFormatting(excel, sheetname, cols=17, rows=2:nrow(GoDF[[x]]), rule=upreg, style = posStyle)
  conditionalFormatting(excel, sheetname, cols=17, rows=2:nrow(GoDF[[x]]), rule=downreg, style = posStyle)
  conditionalFormatting(excel, sheetname, cols=7, rows=2:nrow(GoDF[[x]]), rule="<0.05", style = posStyle)
  conditionalFormatting(excel, sheetname, cols=10, rows=2:nrow(GoDF[[x]]), rule="<0.05", style = posStyle)
}
